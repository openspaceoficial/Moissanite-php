    ...
    if (isset($_POST['cadastro'])) {
        ...
        if ($result->num_rows > 0) {
            ...
            $query = "INSERT INTO usuarios (nome, email, senha, telefone, data_nascimento, endereco, cep, cidade, role) 
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'cliente')";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("ssssssss", $nome, $email, $senha_hash, $telefone, $data_nascimento, $endereco, $cep, $cidade);

            if ($stmt->execute()) {
                // Usuário cadastrado com sucesso
                $_SESSION['usuario_id'] = $stmt->insert_id; // Captura o ID do novo usuário
    ...
    if (isset($_POST['email']) && isset($_POST['senha']) && !isset($_POST['cadastro'])) {
        ...
        if (($email === $admin_email && $senha === $admin_senha) || ($email === $admin_email2 && $senha === $admin_senha2)) {
            ...
            if ($result->num_rows > 0) {
                ...
                // Adicione um var_dump para depurar
                var_dump($usuario);
                ...
                if (password_verify($senha, $usuario['senha'])) {
                    ...
                    // Adicione var_dump para depuração da sessão
                    var_dump($_SESSION);
