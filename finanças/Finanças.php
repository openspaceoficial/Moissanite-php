...
// Aplicação do filtro no botão "Filtrar"
if (isset($_POST['filtrar'])) {
    ...
    $exibirTabela = true; // Exibir a tabela após aplicar o filtro
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $fatu_Moi = $comp_Moi = $fatu_Zir = $comp_Zir = 0;

    // Soma os valores de faturamento da tabela `orcamento`
    $query_orc_moi = "SELECT SUM(ValorPedra) AS TotalFaturamentoM FROM orcamento WHERE TipoPedra = 'Moissanite'";
    $query_orc_zir = "SELECT SUM(ValorPedra) AS TotalFaturamentoZ FROM orcamento WHERE TipoPedra = 'Zirconia'";

    // Soma os valores de custo das tabelas `moissanite` e `zirconia`
    $query_custo_moi = "SELECT SUM(Preco_Custo) AS Total_CustoM FROM moissanite";
    $query_custo_zir = "SELECT SUM(Preco_Custo) AS Total_CustoZ FROM zirconia";

    $result_orc_moi = $conn->query($query_orc_moi);
    $result_orc_zir = $conn->query($query_orc_zir);
    $result_custo_moi = $conn->query($query_custo_moi);
    $result_custo_zir = $conn->query($query_custo_zir);

    // Atualiza o faturamento com os valores da tabela `orcamento`
    if ($result_orc_moi->num_rows > 0) {
        $row_orc_moi = $result_orc_moi->fetch_assoc();
        $fatu_Moi = $row_orc_moi['TotalFaturamentoM'];
    }

    if ($result_orc_zir->num_rows > 0) {
        $row_orc_zir = $result_orc_zir->fetch_assoc();
        $fatu_Zir = $row_orc_zir['TotalFaturamentoZ'];
    }

    // Atualiza os custos com os valores das tabelas `moissanite` e `zirconia`
    if ($result_custo_moi->num_rows > 0) {
        $row_custo_moi = $result_custo_moi->fetch_assoc();
        $comp_Moi = $row_custo_moi['Total_CustoM'];
    }

    if ($result_custo_zir->num_rows > 0) {
        $row_custo_zir = $result_custo_zir->fetch_assoc();
        $comp_Zir = $row_custo_zir['Total_CustoZ'];
    }

    // Calcula os valores
    $somaFaturamento = $fatu_Moi + $fatu_Zir;
    $somaCompras = $comp_Moi + $comp_Zir;
    $despesasTotais = $somaFaturamento - $somaCompras;
    $valorLiquido = $despesasTotais / 2;
    ...
    if (isset($_POST['adicionar'])) {
        ...
        $stmt->bind_param("sssssddi", $tipo, $nome, $assunto, $data, $pagamentos, $preco, $precoParcelas, $parcela);
        if (!$stmt->execute()) {
            die("Erro ao adicionar valores: " . $stmt->error);
        }
        $stmt->close();
    ...
    if (isset($_POST['atualizar'])) {
        ...
        $stmt->bind_param("sssssddii", $tipo, $nome, $assunto, $data, $pagamentos, $preco, $precoParcelas, $parcela, $id);
        if (!$stmt->execute()) {
            die("Erro ao atualizar valores: " . $stmt->error);
        }
        $stmt->close();
    ...
    if (isset($_POST['excluir'])) {
        ...
        $stmt->bind_param("i", $id);
        if (!$stmt->execute()) {
            die("Erro ao excluir registro: " . $stmt->error);
        }
        $stmt->close();
    ...
    if (isset($_POST['marcarPago'])) {
        ...
        $stmt->bind_param("i", $id);
        if (!$stmt->execute()) {
            die("Erro ao marcar registro como Pago: " . $stmt->error);
        }
        $stmt->close();
